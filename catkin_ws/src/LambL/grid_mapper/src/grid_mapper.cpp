...
public:
  ...
  void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    for(float i = angle_mini; i<(angle_max-angle_inc); i=i+angle_inc) {
        x_temp = x + msg->ranges[count]*cos(i);
        y_temp = y - msg->ranges[count]*sin(i);
        count++;
        if(msg->ranges[count] > range_min && msg->ranges[count] < range_max) {
            
            for (float k = 0.2; k < 1.0; k += 0.2) {
                float x_hold = x + k * (x_temp - x);
                float y_hold = y + k * (y_temp - y);
                int x_cell = x_hold + canvas.rows/2;
                int y_cell = y_hold + canvas.cols/2;
                if (canvas.at<char>(x_cell, y_cell) != CELL_ROBOT) {
                    plot(x_hold, y_hold, CELL_FREE);
                }
            }
            int x_cell = x_temp + canvas.rows/2;
            int y_cell = y_temp + canvas.cols/2;
           if(canvas.at<char>(x_cell, y_cell) == CELL_UNKNOWN && msg->ranges[count] < range_max)
            plot(x_temp, y_temp, CELL_OCCUPIED);
        }
    }
};
