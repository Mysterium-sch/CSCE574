...
class RandomWalk {
 public:
  ...
  float calculate_desired_heading(){
  ...
  void spin() {
    ...
    while (ros::ok()) {  // Keep spinning loop until user presses Ctrl+C
      ...
      if(start) {
        ...
        // error time
        float error = closestRange - xd;
        in_eror += error;
        float dev_error = error - prev_error;
        float turn = p * error + d * dev_error;
        prev_error = error;
        std::cout << "Turn rate from error: " << turn << "\n\n";
        move(0.2, turn);
      }
      /////////////////////// ANSWER CODE END ///////////////////
